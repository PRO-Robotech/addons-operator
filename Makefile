...
KIND_CLUSTER ?= addon-operator-test-e2e
...
build: manifests generate fmt vet ## Build manager binary.
	go build -o bin/manager ./cmd/addons-operator

.PHONY: run
run: manifests generate fmt vet ## Run a controller from your host.
	go run ./cmd/addons-operator
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name addon-operator-builder
	$(CONTAINER_TOOL) buildx use addon-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm addon-operator-builder
	rm Dockerfile.cross
