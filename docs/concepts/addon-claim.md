...
|------|-----|-------------|----------|
| `name` | string | Да | Имя аддона. Используется как имя Addon в infra-кластере. **Неизменяемо после создания** |
| `version` | string | Да | Версия аддона |
| `cluster` | string | Да | Имя целевого клиентского кластера (используется в рендеринге шаблона). **Неизменяемо после создания** |
...
Поля `name` и `cluster` **нельзя изменить** после создания AddonClaim. Это защита от несогласованности: имя Addon в infra-кластере и целевой кластер определяются при создании и не должны меняться, так как это приведёт к потере связи с уже развёрнутыми ресурсами.

При попытке изменить эти поля webhook вернёт ошибку:

```
Error: admission webhook denied the request: name is immutable
```

Для изменения `name` или `cluster` удалите AddonClaim и создайте новый.
