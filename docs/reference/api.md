...
Когда `backend.finalizer: true`, на Argo CD Application устанавливается финалайзер `resources-finalizer.argocd.argoproj.io`. При удалении Application Argo CD сначала удалит все созданные им ресурсы (Deployments, Services и т.д.), и только затем удалит сам объект Application.
...
| Поле | Тип | Обязательно | Описание |
|------|-----|-------------|----------|
| `name` | string | Да | Имя Addon-ресурса в удалённом кластере (1–253 символа). **Неизменяемо** после создания (CEL + webhook) |
...
|------|-----|----------|
| `observedGeneration` | int64 | Последняя обработанная spec.generation |
| `ready` | *bool | Удалённый Addon в состоянии Ready. `*bool` для корректной CAPI-сериализации (`nil` = absent, `false` = not ready, `true` = ready) |
...
| `initialized` | *bool | CAPI: control plane инициализирован (только при аннотации `external-status/type`) |
| `initialization` | [Initialization](#initialization) | CAPI v1beta2: статус инициализации (только при аннотации) |
| `externalManagedControlPlane` | *bool | CAPI: control plane управляется внешне (только при аннотации) |
| `version` | string | CAPI: версия из `variables.version` (только при аннотации) |
| `conditions` | []Condition | Conditions текущего состояния |
...
| Поле | Тип | Описание |
|------|-----|----------|
| `controlPlaneInitialized` | *bool | Первый экземпляр control plane готов |
...
|-----------|----------|----------|
| `external-status/type` | `controlplane` | Активирует заполнение CAPI-совместимых полей в status |
...
| Путь | Описание |
|------|----------|
| `.Vars.<key>` | Быстрый доступ к переменным из `spec.variables` (рекомендуется) |
| `.Values.spec.addon.name` | Имя Addon в удалённом кластере |
| `.Values.spec.variables.<key>` | Полный путь к переменным |
...
> **Примечание:** Поле `metadata.name` в шаблоне всегда переопределяется контроллером значением `spec.addon.name` из AddonClaim.
...
| `jsonPath` | string | Да | - | Путь к значению (RFC 9535 JSONPath синтаксис) |
...
| `keep` | *bool | Нет | true | Фиксация (latch) criterion после совпадения. Если true — criterion фиксируется и больше не перевычисляется. Если false — перевычисляется каждый цикл |

**Семантика keep (latch):**

Каждый criterion с `keep=true` (или не указан) фиксируется индивидуально при первом совпадении правила. После этого он пропускается при перевычислении — считается автоматически выполненным. Criteria с `keep=false` продолжают перевычисляться каждый цикл reconcile.

Это позволяет комбинировать: `keep=true` на зависимости (cert-manager Ready) и `keep=false` на динамическом условии (replicas >= 3). Зависимость фиксируется и не вызовет каскадный сбой при обновлении, а динамическое условие продолжает реагировать на изменения.

Если ВСЕ criteria имеют `keep=false`, правило никогда не фиксируется.

Поле `keep` является **неизменяемым** после создания — изменение эффективного значения keep запрещено webhook'ом.
...
| `finalizer` | *bool | Нет | nil | Каскадное удаление ресурсов при удалении Application |
...
| `$.status.conditions[?@.type=='Ready'].status` | Filter по полю |
